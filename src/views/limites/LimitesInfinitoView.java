...
import views.cuestionarios.CuestionarioInfinitoView;

public class LimitesInfinitoView {
    private ViewManager viewManager;
    private LimitesMenuView parentView;

    public LimitesInfinitoView(ViewManager viewManager, LimitesMenuView parentView) {
    ...
    public VBox createView() {
        ...
        Text title = new Text("AL INFINITO");
        title.setFont(Font.font("Segoe UI", FontWeight.BOLD, 24));
        title.setFill(AppConstants.TITLE_COLOR);

        Text subtitle = new Text("Límites al infinito");
    ...
    private VBox createDefinicionSection() {
        ...
        Label title = new Label("Límites al Infinito");
        title.setFont(Font.font("Segoe UI", FontWeight.BOLD, 18));
        title.setTextFill(AppConstants.TITLE_COLOR);

        TextArea definicionText = new TextArea(
                "Los límites al infinito analizan el comportamiento de una función f(x) cuando x crece indefinidamente (x → ∞) o decrece indefinidamente (x → -∞).\n\n"
                        +
                        "Reglas básicas para funciones racionales (polinomio/polinomio):\n" +
                        "• Si grado(num) < grado(den) → El límite es 0.\n" +
                        "• Si grado(num) = grado(den) → El límite es el cociente de los coeficientes principales.\n" +
                        "• Si grado(num) > grado(den) → El límite es ∞ o -∞.\n\n" +
                        "También existen los límites infinitos, donde el resultado del límite es ∞ (asíntotas verticales).");
    ...
    private VBox createCuestionarioSection() {
        ...
        Label subTitle = new Label("AL INFINITO");
        subTitle.setFont(Font.font("Segoe UI", FontWeight.NORMAL, 14));
        ...
        resolverBtn.setOnAction(e -> {
            CuestionarioInfinitoView cuestionarioView = new CuestionarioInfinitoView(viewManager, this);
            viewManager.getRoot().getChildren().clear();
    ...
    private void reproducirVideo() {
        YouTubePlayer.playVideo(
                ...
                "Video Explicativo - Límites al Infinito",
                "Comprende el comportamiento de funciones cuando x tiende al infinito");
    }
    ...
    private VBox createConceptosSection() {
        ...
        itemBox.setOnMouseClicked(e -> {
            EjerciciosInfinitoView ejerciciosView = new EjerciciosInfinitoView(viewManager, this);
            ejerciciosView.show();
        });
