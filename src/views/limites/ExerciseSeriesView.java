...
public class ExerciseSeriesView {
    ...
    public VBox createView() {
        ...
        Text subtitle = new Text("Ejercicios Resueltos (" + (currentExerciseIndex + 1) + "/" + exercises.length + ")");
        ...
        // Exercise Card
        VBox card = createExerciseCard(currentExerciseIndex);
        ...
        if (currentExerciseIndex < exercises.length - 1) {
            ...
            Button practiceButton = new Button("Ir a Práctica →");
            practiceButton.setStyle("-fx-background-color: #3EB566; -fx-text-fill: white; " +
                    "-fx-font-weight: bold; -fx-background-radius: 20; -fx-padding: 12 25; -fx-font-size: 14; " +
                    "-fx-cursor: hand; -fx-effect: dropshadow(gaussian, rgba(62,181,102,0.3), 10, 0, 0, 3);");
            practiceButton.setOnAction(e -> {
                PracticeView practiceView = new PracticeView(viewManager);
                viewManager.getRoot().getChildren().clear();
                viewManager.getRoot().getChildren().add(practiceView.createView());
            });
            navigationBox.getChildren().add(practiceButton);
        }
