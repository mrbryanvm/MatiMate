...
import views.cuestionarios.CuestionarioNotablesView;

public class LimitesNotablesView {
    private ViewManager viewManager;
    private LimitesMenuView parentView;

    public LimitesNotablesView(ViewManager viewManager, LimitesMenuView parentView) {
    ...
    private VBox createDefinicionSection() {
        ...
        Label title = new Label("Fórmulas Importantes");
        title.setFont(Font.font("Segoe UI", FontWeight.BOLD, 18));
        title.setTextFill(AppConstants.TITLE_COLOR);

        TextArea definicionText = new TextArea(
                "Los límites notables son límites cuyo resultado ya es conocido y se usan para resolver problemas más complejos.\n\n"
                        +
                        "1. Límite trigonométrico fundamental:\n" +
                        "   lim(x→0) (sen x)/x = 1\n\n" +
                        "2. Límite exponencial:\n" +
                        "   lim(x→∞) (1 + 1/x)^x = e\n\n" +
                        "3. Límite logarítmico:\n" +
                        "   lim(x→0) (ln(1+x))/x = 1\n\n" +
                        "Estos límites son muy útiles para resolver indeterminaciones del tipo 0/0 o 1^∞.");
    ...
    private VBox createCuestionarioSection() {
        ...
        Label subTitle = new Label("NOTABLES");
        subTitle.setFont(Font.font("Segoe UI", FontWeight.NORMAL, 14));
        ...
        resolverBtn.setOnAction(e -> {
            CuestionarioNotablesView cuestionarioView = new CuestionarioNotablesView(viewManager, this);
            viewManager.getRoot().getChildren().clear();
    ...
    private void reproducirVideo() {
        YouTubePlayer.playVideo(
                "https://www.youtube.com/watch?v=IlGCkcRblOY",
                "Video Explicativo - Límites Notables",
                "Domina los límites trigonométricos y exponenciales más importantes");
    }
    ...
    private VBox createConceptosSection() {
        ...
        itemBox.setOnMouseClicked(e -> {
            EjerciciosNotablesView ejerciciosView = new EjerciciosNotablesView(viewManager, this);
            ejerciciosView.show();
        });
